...
| Alta | El contacto empezaba por un formulario genérico y no ofrecía un recorrido claro para agendar. | Correo como canal principal, videollamada secundaria y formulario con ciudad y ejemplar opcionales. | La conversación empieza por una vía verificable y conserva la agenda como alternativa. |
...
| Consulta | ¿Cómo hablo con ustedes sobre este ejemplar? | Correo contextual desde cada ficha o formulario con el perfil preseleccionado; la agenda permanece como opción secundaria. Los reservados conservan su estado en la medición. |
...
- Se mantienen las URLs principales, las imágenes sociales, GTM, el endpoint de Formspree, el correo y la agenda existentes.
...
Se preserva `xolos_generate_lead` y su contrato: un clic o envío válido es una activación, no una prueba de contacto real. Se añaden eventos independientes:
...
| `qualified_contact_intent` | Activación explícita de precio, perfil, videollamada o formulario. | `lead_channel`, `lead_intent`, `cta_location`, `profile`, `profile_status`, `page_type`, `lang`. |
| `contact_form_submit` | Respuesta HTTP satisfactoria de Formspree. | Contexto no personal del formulario y página. |

Los eventos nuevos no incluyen el contenido del formulario, email, ciudad ni query strings. `contact_form_submit` incluye el ID público canónico del perfil solo cuando la persona seleccionó uno. No se ha modificado la configuración de GTM/GA4: hay que configurar el envío de estos dos eventos desde el contenedor para verlos en GA4. El éxito de Formspree confirma la aceptación de la solicitud por el servicio, no su lectura por el equipo ni una entrega de correo verificada.
...
- Diez pruebas de comportamiento de filtros y formulario: consulta conocida/desconocida, entrada inválida, envío, perfil canónico opcional, doble clic, error HTTP, red, timeout, reintento y eventos sin datos personales.
- Se mantiene aprobada la suite existente de seguimiento de leads, actualizada para comprobar el CTA flotante de precio por correo.
...
No se ejecutó prueba visual en navegador, navegación real de agenda/correo, entrega real del formulario ni Lighthouse sobre esta rama. No se atribuyen mejoras numéricas de velocidad, ranking o conversión sin medición. Antes de aprobar el merge conviene revisar escritorio/móvil, zoom al 200 %, menú/galerías por teclado y las superficies de contacto en un navegador real.
...
3. **Continuidad operativa.** Conciliar las consultas con compatibilidad, reserva, entrega y seguimiento mediante el registro comercial existente; no deducir ventas a partir de aperturas del cliente de correo.
